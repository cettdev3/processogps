...
#     return True
# links do drive para cada solicitação
# SolTransporteLinkDoDrive
# solDiariasProfLinkDoDrive
# solDiariasMotoristaLinkDoDrive
# adiantEqExecLinkDoDriveComb
# adiantEqExecLinkDoDriveIns
def keyEqualsValue(task, key, value):
    return task.variables[key].value == value if key in task.variables else False

def KeyValueIfExists(task, key):
    return task.variables[key].value if key in task.variables else ""
...
def mountSolicitacaoAluguelVeiculo(task):
    curso = KeyValueIfExists(task, 'cursos')
    data_inicio = KeyValueIfExists(task, 'data_inicio')
    data_final = KeyValueIfExists(task, 'data_fim')
    cidade_realizacao = KeyValueIfExists(task,'cidade_realizacao')
    endereco_realizacao = KeyValueIfExists(task, 'endereco_realizacao')
    ...
    SolTransporteLinkDoDrive = task.variables["SolTransporteLinkDoDrive"].value
    ...
    textoAluguelVeiculo += mountParte("Link do Drive", SolTransporteLinkDoDrive)
...
def mountSolicitacaCompraPassagem(task):
    ...
    textoCompraPassagem += mountParte("SolTransporteLinkDoDrive", SolTransporteLinkDoDrive)
...
def mountSolicitacaDiariaProf(task):
    ...
    solDiariasProfLinkDoDrive = task.variables['solDiariasProfLinkDoDrive'].value
    ...
    textoDiariaProf += mountParte("Link do Drive", solDiariasProfLinkDoDrive)
...
def mountSolicitacaDiariaMotorista(task):
    ...
    solDiariasMotoristaLinkDoDrive = task.variables['solDiariasMotoristaLinkDoDrive'].value
    ...
    textoDiariaAuxExt += mountParte("Link do Drive", solDiariasMotoristaLinkDoDrive)
...
def mountSolicitacaoAdiantamentoCom(task):
    ...
    adiantEqExecLinkDoDriveComb = task.variables['adiantEqExecLinkDoDriveComb'].value
    ...
    textoAdiantamentoCombustivel += mountParte("Link do Drive", adiantEqExecLinkDoDriveComb)
...
def mountSolicitacaoAdiantamentoInsumos(task):
    ...
    adiantEqExecLinkDoDriveIns = task.variables['adiantEqExecLinkDoDriveIns'].value
    ...
    textoAdiantamentoCombustivel += mountParte("Link do Drive", adiantEqExecLinkDoDriveIns)

    return textoAdiantamentoCombustivel

def mountItemSolicitacaoKits(task, numero):
    curso = KeyValueIfExists(task, 'cursos')
    data_inicio = KeyValueIfExists(task, 'data_inicio')
    data_final = KeyValueIfExists(task, 'data_fim')
    cidade_realizacao = KeyValueIfExists(task,'cidade_realizacao')
    ...
    solProtocInstRefer = task.variables['solProtocInstRefer'].value
    # formAuxExtJustificativa = task.variables['formAuxExtJustificativa'+numero].value
    reqInsumoKitsItemNome = task.variables['reqInsumoKitsItemNome'+numero].value
    reqInsumoKitsItemDescricao = task.variables['reqInsumoKitsItemDescricao'+numero].value

    textoItemSolicitacaoInsumoKits = mountParte("Nome", reqInsumoKitsItemNome)
    textoItemSolicitacaoInsumoKits += mountParte("Descrição", reqInsumoKitsItemDescricao)

    return textoItemSolicitacaoInsumoKits

def mountSolicitacaoInsumosKits(task):
    curso = KeyValueIfExists(task, 'cursos')
    data_inicio = KeyValueIfExists(task, 'data_inicio')
    data_final = KeyValueIfExists(task, 'data_fim')
    cidade_realizacao = KeyValueIfExists(task,'cidade_realizacao')
    ...
    solProtocInstRefer = KeyValueIfExists(task,'solProtocInstRefer')    
    reqInsumoKitsNomeSolicitante = task.variables["reqInsumoKitsNomeSolicitante"].value

    textoSolicitacaoItemSolicitacaoInsumosKits = mountParte("Solicitação de Insumos Para Kits")
    textoSolicitacaoItemSolicitacaoInsumosKits += reqInsumoKitsNomeSolicitante + " solicita Insumos para produção de kits, "
    textoSolicitacaoItemSolicitacaoInsumosKits += "para o curso "
    textoSolicitacaoItemSolicitacaoInsumosKits += curso+", a ser realizado na cidade de " + cidade_realizacao + " (GO), cujas aulas serão de "  +data_inicio + " a " + data_final
    textoSolicitacaoItemSolicitacaoInsumosKits += ", vinculado ao COTEC"+ solProtocInstRefer+", para o GPS."

    textoSolicitacaoItemSolicitacaoInsumosKits += mountParte("ITENS")
    if KeyValueIfExists(task, "qtdItemsSolicitacaoInsumoKits"):
        for i in range(int(task.variables['qtdItemsSolicitacaoInsumoKits'].value)):
            textoSolicitacaoItemSolicitacaoInsumosKits += "<br>"
            textoSolicitacaoItemSolicitacaoInsumosKits += mountItemSolicitacaoKits(task, str(i+1))

    return textoSolicitacaoItemSolicitacaoInsumosKits


def mountDadosSolicitante(task, tipo = ""):
    curso = KeyValueIfExists(task, 'cursos')
    data_inicio = KeyValueIfExists(task, 'data_inicio')
    data_final = KeyValueIfExists(task, 'data_fim')
    cidade_realizacao = KeyValueIfExists(task,'cidade_realizacao')
    ...
    solProtocEmail = KeyValueIfExists(task,'solProtocEmail') if tipo != "Solicitação de Insumos Para Kits"  else ""
    solProtocNomeCompleto = KeyValueIfExists(task,'solProtocNomeCompleto') if tipo != "Solicitação de Insumos Para Kits"  else task.variables['reqInsumoKitsNomeSolicitante'].value
    solProtocTelefone = KeyValueIfExists(task,'solProtocTelefone') if tipo != "Solicitação de Insumos Para Kits"  else ""
    solProtocInstRefer = KeyValueIfExists(task,'solProtocInstRefer') if tipo != "Solicitação de Insumos Para Kits"  else ""
    solProtocProjeto = KeyValueIfExists(task,'solProtocProjeto') if tipo != "Solicitação de Insumos Para Kits"  else ""

    subject = ""
    if curso:
        subject += curso+"-"
    if cidade_realizacao:
        subject += cidade_realizacao+"-"
    if solProtocNomeCompleto:
        subject += solProtocNomeCompleto
...
def setVariables(task, idsGerados):
    ticketIdSolicitacaoTransporte = idsGerados["ticketIdSolicitacaoTransporte"] 
    ticketIdSolicitacaoDiaria = idsGerados["ticketIdSolicitacaoDiaria"]
    ticketIdSolicitacaoAdiantamento = idsGerados["ticketIdSolicitacaoAdiantamento"]
    ticketIdSolicitacaoInsumosKits = idsGerados["ticketIdSolicitacaoInsumosKits"]
    variables = {}
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for task in tasks:
            ...
            if keyEqualsValue(task, "tipoSolTransporte", "veiculo"):
                mensagemSolTransporte = mountSolicitacaoAluguelVeiculo(task)
            if keyEqualsValue(task, "tipoSolTransporte", "servico"):
                mensagemSolTransporte = mountSolicitacaContratServico(task)
            if keyEqualsValue(task, "tipoSolTransporte", "passagem"):
                mensagemSolTransporte = mountSolicitacaCompraPassagem(task)

            dadosSolicitante = mountDadosSolicitante(task)

            cpfDiariaProf = KeyValueIfExists(task,'solDiariasProfCpf')
            mensagemSolDiariaProf = ""
            if cpfDiariaProf:
                mensagemSolDiariaProf = mountSolicitacaDiariaProf(task)

            solicitanteInsumosKits = KeyValueIfExists(task,'reqInsumoKitsNomeSolicitante')
            mensagemSolInsumosKits = ""
            if solicitanteInsumosKits:
                mensagemSolInsumosKits = mountSolicitacaoInsumosKits(task)

            cpfAdiantComb = KeyValueIfExists(task,'adiantEqExecCpfComb')
            mensagemSolAdiantComb = ""
            if cpfAdiantComb:
                mensagemSolAdiantComb = mountSolicitacaoAdiantamentoCom(task)

            cpfAdiantInsumo = KeyValueIfExists(task,'adiantEqExecCpfIns')
            mensagemSolAdiantInsumo = ""
            if cpfAdiantInsumo:
                mensagemSolAdiantInsumo = mountSolicitacaoAdiantamentoInsumos(task)

            cpfDiariaMotorista = KeyValueIfExists(task,'solDiariasMotoristaCpf')
            ...
            if KeyValueIfExists(task, "qtdAuxExt"):
            ...
            if mensagem != "" and not KeyValueIfExists(task, "ticketIdSolicitacaoTransporte"):
            ...
            if mensagem != "" and not KeyValueIfExists(task, "ticketIdSolicitacaoDiaria"):
            ...
            if mensagem != ""  and not KeyValueIfExists(task, "ticketIdSolicitacaoAdiantamento"):
            ...
            mensagem = mensagemSolInsumosKits
            response = ""
            ticketIdSolicitacaoInsumosKits = ""
            if mensagem != "":
                dadosSolicitante = mountDadosSolicitante(task, "Solicitação de Insumos Para Kits")
                response = saveOnJumla(dadosSolicitante, mensagem, "Solicitação de Insumos Para Kits")
                response = json.loads(response.content)
                ticketIdSolicitacaoInsumosKits = response["ticketid"]

            idsGerados = {
                "ticketIdSolicitacaoTransporte": ticketIdSolicitacaoTransporte,
                "ticketIdSolicitacaoDiaria": ticketIdSolicitacaoDiaria,
                "ticketIdSolicitacaoAdiantamento": ticketIdSolicitacaoAdiantamento,
                "ticketIdSolicitacaoInsumosKits": ticketIdSolicitacaoInsumosKits
            }
            variables = setVariables(task, idsGerados)
